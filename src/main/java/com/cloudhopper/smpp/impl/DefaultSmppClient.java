...
public class DefaultSmppClient implements SmppClient {
	 ...
	 * monitorExecutors. The maximum number of IO worker threads across any client sessions created with this SmppClient
	 * will be Runtime.getRuntime().availableProcessors(). An Executors.newCachedDaemonThreadPool will be used for IO
	 * worker threads.
	 */
	 ...
	 *
	 * @param executors The executor that IO workers will be executed with. An Executors.newCachedDaemonThreadPool() is
	 ...
	 * @param executors The executor that IO workers will be executed with. An Executors.newCachedDaemonThreadPool() is
	 * recommended. The max threads will never grow more than expectedSessions if NIO sockets are used.
	 * @param expectedSessions The max number of concurrent sessions expected to be active at any time. This number
	 * controls the max number of worker threads that the underlying Netty library will use. If processing occurs in a
	 * sessionHandler (a blocking op), be <b>VERY</b> careful setting this to the correct number of concurrent sessions
	 * you expect.
	 */
	 ...
	 * sessionHandler (a blocking op), be <b>VERY</b> careful setting this to the correct number of concurrent sessions
	 * you expect.
	 * @param monitorExecutor The scheduled executor that all sessions will share to monitor themselves and expire
	 * requests. If null monitoring will be disabled.
	 */
	...
	protected DefaultSmppSession doOpen(SmppSessionConfiguration config, SmppSessionHandler sessionHandler)
		...
		Channel channel = createConnectedChannel(config.getHost(), config.getPort(), config.getConnectTimeout(), config.getLocalAddress());
	...
	protected Channel createConnectedChannel(String host, int port, long connectTimeoutMillis)
			throws SmppTimeoutException, SmppChannelException, InterruptedException {
		return createConnectedChannel(host, port, connectTimeoutMillis, null);
	}

	protected Channel createConnectedChannel(String host, int port, long connectTimeoutMillis, String localAddr)
			throws SmppTimeoutException, SmppChannelException, InterruptedException {
		...
		if (localAddr == null) {
			connectFuture = this.clientBootstrap.connect(socketAddr);
		}
		else {
			connectFuture = this.clientBootstrap.connect(
					socketAddr,
		...
		if (connectFuture.isCancelled()) {
			...
			if (connectFuture.getCause() instanceof org.jboss.netty.channel.ConnectTimeoutException) {
				throw new SmppChannelConnectTimeoutException(
						"Unable to connect to host [" + host + "] and port [" + port + "] within "
						+ connectTimeoutMillis + " ms", connectFuture.getCause()
				);
			}
			else {
				throw new SmppChannelConnectException(
						"Unable to connect to host [" + host + "] and port [" + port + "]: "
